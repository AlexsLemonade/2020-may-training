...
Sequence Read Archive (SRA) is a public repository of sequencing data.  
...
SRAdb has an [sqlite file](https://www.sqlite.org/aff_short.html) with all the metadata information we need.
The `SRAmetadb.sqlite` file contains the database that we query to get the metadata we need.

The Tidyverse includes the [`dbplyr` package](https://dbplyr.tidyverse.org/) made for working with _databases_ (that's what the db stands for).
...
Let's first look at how we can use `stringr::str_replace` to get only the relevant information we want. 
...
```{r}
cleaned_sample_df <- cleaned_sample_df %>%
  # We can use the same character vector as before to 
